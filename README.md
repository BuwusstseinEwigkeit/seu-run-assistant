...
1. 下载仓库后，双击根目录的 `index.html`，用浏览器打开（推荐 Chrome / Edge）。无需安装任何依赖。

   ![仓库根目录](assets/guide-folder-root-v2.png)

2. 双击 `tools/token-tool/get-token.bat` 启动本地桥接服务，并保持该窗口开启。提取 Token 和提交记录都经由它完成。
3. 按页面上的教程操作。
...
场地列表**不再写死在页面里**，而是在校验 Token 后由页面调用服务端
`/api/blade-exercise/v2/exerciseRecord/listRule` 实时拉取，当前共 7 个：

九龙湖-桃园田径场 · 九龙湖-梅园田径场 · 九龙湖-橘园田径场 · 四牌楼-四牌楼田径场 ·
四牌楼-校东田径场 · 无锡校区 · 丁家桥校区

> 以页面「运动场地」下拉框显示为准。轨迹会直接使用服务端下发的场地多边形（`latlngs`），
> 所以场地增删不需要改代码。
>
> 早先版本写死的旧列表（小营田径场、四牌楼体育场等）属于上一代部署，其 `rule_id` / `plan_id`
> 都已失效，用它们提交会得到 `500 服务器异常`。
